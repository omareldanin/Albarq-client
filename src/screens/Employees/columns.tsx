...
export const columns: ColumnDef<Employee>[] = [
    ...
    },
    {
        ...
        cell: ({ row }) => {
            ...
            if (role === "CLIENT_ASSISTANT") {
                const stringifiedManagedStores = managedStores.map((store) => store.id.toString());
                return <AssignClientAssistantToStores id={id} managedStores={stringifiedManagedStores} />;
            }
            return "--";
        }
    },
...

    {
        ...
        cell: ({ row }) => {
            ...
            const [inquiryBranchesOpened, { open: openInquiryBranches, close: closeInquiryBranches }] =
                useDisclosure(false);
            const [inquiryStatusesOpened, { open: openInquiryStatuses, close: closeInquiryStatuses }] =
                useDisclosure(false);
            const [
                inquiryGovernoratesOpened,
                { open: openInquiryGovernorates, close: closeInquiryGovernorates }
            ] = useDisclosure(false);
            const [inquiryStoresOpened, { open: openInquiryStores, close: closeInquiryStores }] =
                useDisclosure(false);
            const [inquiryCompaniesOpened, { open: openInquiryCompanies, close: closeInquiryCompanies }] =
                useDisclosure(false);
            const [inquiryLocationsOpened, { open: openInquiryLocations, close: closeInquiryLocations }] =
                useDisclosure(false);
            const [isMenuOpen, setMenuOpen] = useState(false);

            const isInquiryRole = role === "INQUIRY_EMPLOYEE";
            const stringifiedManagedBranches = row.original.inquiryBranches.map((branch) =>
                branch.id.toString()
            );

            const stringifiedLocations = row.original.inquiryLocations.map((location) =>
                location.id.toString()
            );
            const stringifiedStores = row.original.inquiryStores.map((store) => store.id.toString());

            const stringifiedCompanies = row.original.inquiryCompanies.map((company) =>
                company.id.toString()
            );
