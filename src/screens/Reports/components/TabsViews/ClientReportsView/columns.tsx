...
const getReportName = (type: IReport["type"], row: IReport): string => {
    const { repositoryReport, branchReport, clientReport, deliveryAgentReport, governorateReport } = row;
    ...
    switch (type) {
        ...
        case "GOVERNORATE":
            return (governorateReport && governorateArabicNames[governorateReport?.governorate]) || "";
        case "COMPANY":
            return "";
        default:
            return "";
...
export const columns: ColumnDef<IReport>[] = [
    {
            ...
            return (
                <Checkbox
                    checked={
                        ...
                        table
                            .getRowModel()
                            .rows.every((row) => isClientReportExist(row.original.id.toString()))
                    }
                    onChange={(event) => {
        ...
        cell: ({ row }) => {
            const { addClientReport, deleteClientReport, isClientReportExist } = useClientReportStoreStore();
            return (
                <Checkbox
