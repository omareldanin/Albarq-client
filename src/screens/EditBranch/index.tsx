...
import { type EditBranchPayload, editBranchService } from "@/services/editBranchService";
...
export const EditBranch = () => {
    ...
    const { id = "" } = useParams();
    const { data: branchDetails, isLoading, isError } = useBranchDetails(Number.parseInt(id));
    ...
    const handleSubmit = (values: z.infer<typeof editBranchSchema>) => {
        const enBranch = governorateArray.find((item) => item.label === values.location);
        if (!enBranch) {
    ...
    return (
        <AppLayout isLoading={isLoading} isError={isError}>
            ...
            >
                ...
                />
                <Button loading={isEditing} disabled={isEditing} type="submit" fullWidth mt="xl" size="md">
                    تعديل
                </Button>
                <Button
                    type="reset"
